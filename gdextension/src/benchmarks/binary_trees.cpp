...
void CPPBenchmarkBinaryTrees::calculate_binary_trees(int input) {
	...
	UtilityFunctions::print("stretch tree of depth ", stretch_depth, "\t check: ", stretch_tree->check());
	delete stretch_tree;
	...
	for (int depth = min_depth; depth < max_depth; depth += 2) {
		...
		UtilityFunctions::print(iterations, "\t trees of depth ", depth, "\t check: ", check);
	}

	UtilityFunctions::print("long lived tree of depth ", max_depth, "\t check: ", long_lived_tree->check());
