...
void CPPBenchmarkStringFormat::benchmark_complex_variable_concatenate() {
	for (int i = 0; i < iterations; i++)
		"Hello " + engine_name + "!\nA few examples of formatting: " + UtilityFunctions::str(some_integer) + ", " + UtilityFunctions::str(some_float).pad_decimals(2) + ", " + UtilityFunctions::str(some_vector2i);
}
...
void CPPBenchmarkStringFormat::benchmark_complex_variable_method() {
	for (int i = 0; i < iterations; i++) {
		...
		dict["a_float"] = UtilityFunctions::str(some_float).pad_decimals(2);
