...
function displayGraph(targetDivID, graphID, type="full", filter="") {
  ...
  const allBenchmarks = Database.benchmarks.sort((a, b) => (`${a.date}.${a.commit}` > `${b.date}.${b.commit}`))
  ...
  allBenchmarks.forEach((benchmark, count) => {
    ...
    benchmark.benchmarks.forEach((instance) => {
      ...
      instanceKey = instanceKey.slice(graph["benchmark-path-prefix"].length+1)
  ...
  if (type === "compact") {
    ...
    const comparedTo = outputSerie.slice(-totalConsideredCount, -lastElementsCount)
    const avgLast = lastElements.reduce((a, b) => a + b) / lastElements.length;
    const avgComparedTo = comparedTo.reduce((a, b) => a + b) / comparedTo.length;
  ...
  var options = {
    ...
    colors: type === "compact" ? (customColor ? [customColor] : ['#4ecdc4']) : undefined,
    xaxis: {
      categories: xAxis,
      labels: {
        show: type !== "compact",
